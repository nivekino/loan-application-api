...
export async function connectToDatabase(
  ...
  const attemptConnection = async (
    ...
    try {
      ...
      if (attemptsLeft <= 0) {
        if (isError(error)) {
          logger.error(
            `Mongoose connection error after final retry: ${error.message}`,
            error
          );
        } else {
        ...
        if (isError(error)) {
          logger.warn(
            `Mongoose connection attempt failed, retrying in ${retryDelay}ms... Attempts left: ${attemptsLeft}`,
            error
          );
        } else {
          logger.warn(
  ...
  try {
    ...
    if (isError(error)) {
      logger.error(
        `Unable to connect to Mongoose after retries: ${error.message}`,
        error
      );
    } else {
